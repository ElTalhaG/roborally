...
class Repository implements IRepository {
	...
	public boolean createGameInDB(Board game) {
		if (game.getGameId() == null) {
			...
			try {
				...
				// The game name is generated from the current date in this version.
				...
				/* DB4a is not part of this submission stage.
				...
				if (rs.next()) {
					...
					// No matching game row was found while updating the current player.
				}
				rs.close();
				...
				// Basic error logging is used for database update failures.
				e.printStackTrace();
				...
				try {
					...
					// Basic error logging is used for rollback failures.
					e1.printStackTrace();
				}
			}
		} else {
	...
	public boolean updateGameInDB(Board game) {
		...
		try {
			...
			if (rs.next()) {
				...
				// No matching game row was found while updating the game state.
			}
			rs.close();
	...
	public Board loadGameFromDB(int id) {
		...
		try {
			...
			int playerNo = -1;
			if (rs.next()) {
				// The board name is not stored in the database in this project version,
				// so the default board is used when loading from the database.
				game = BoardFactory.getInstance().createBoard(null);
				if (game == null) {
					return null;
				}
				playerNo = rs.getInt(GAME_CURRENTPLAYER);
				// The game name is not restored from the database in this project version.
				game.setPhase(Phase.values()[rs.getInt(GAME_PHASE)]);
				game.setStep(rs.getInt(GAME_STEP));
			} else {
				// No game with the given id was found in the database.
				return null;
			}
			rs.close();
			...
			if (playerNo >= 0 && playerNo < game.getPlayersNumber()) {
				...
				// The stored current player index was outside the valid range.
				return null;
			}
			...
			*/
			...
			// Basic error logging is used for database load failures.
			e.printStackTrace();
			System.err.println("Some DB error");
		}
		return null;
	...
	public List<GameInDB> getGames() {
		// This version fetches all stored games because that is enough for the course project.
		...
		try {
			...
			// Basic error logging is used for database list failures.
			e.printStackTrace();
		}
		return result;		
	}
	...
	private void createPlayersInDB(Board game) throws SQLException {
		// This method assumes that the board state is internally consistent.
	...
	private void loadPlayersFromDB(Board game) throws SQLException {
		...
		while (rs.next()) {
			...
			if (i++ == playerId) {
				// This method assumes that the stored player ordering is valid.
				...
				// The database returned a player sequence with a missing id.
	...
	private void updatePlayersInDB(Board game) throws SQLException {
		...
		while (rs.next()) {
			int playerId = rs.getInt(PLAYER_PLAYERID);
			// This method assumes that the player ids in the database are valid.
			Player player = game.getPlayer(playerId);
			// rs.updateString(PLAYER_NAME, player.getName()); // not needed: player's names does not change
			rs.updateInt(PLAYER_POSITION_X, player.getSpace().x);
			rs.updateInt(PLAYER_POSITION_Y, player.getSpace().y);
			rs.updateInt(PLAYER_HEADING, player.getHeading().ordinal());
			// Player count changes are not handled in this project version.
			rs.updateRow();
		}
		rs.close();
		
		// A full consistency check after player updates is not added in this project version.
	}
	...
	private PreparedStatement getInsertGameStatementRGK() {
		if (insert_game_stmt == null) {
			...
			try {
				...
				// Basic error logging is used for prepared statement creation failures.
				e.printStackTrace();
			}
		}
